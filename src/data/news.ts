    ...
    data: [
      {
        ...
        details: [
          ...
          { day: 19, importance: "low", summary: "些細なパフォーマンスの向上", news: "使用しているアイコンを静的アセットに変更しようと試みましたが・・・、初回ロード後はCDNでキャッシュされているならパフォーマンスにはあまり問題ないんじゃないかな、と思って現状維持にしました。<br>変更点としては、タイトル画像の文字の表示がどうしてもデスクトップなどの大きな画面では画質悪く見えてしまっていたのでそこだけはcloudinaryに、かなり大きめのサイズのものをアップロードし、それを自動的に各デバイスに最適化したものを配信する形にしてみました。"}
        ]
      }
    ]
  }
